...
describe 'Ridgepole::Client#diff -> migrate', condition: '>= 6.0' do
  ...
  context 'when add list default partition' do
    let(:actual_dsl) do
      erbh(<<-ERB)
        create_table "list_partitions", id: false, force: :cascade, options: "PARTITION BY LIST(id)" do |t|
          t.integer "id", null: false
  ...
  context 'when add range default partition' do
    let(:actual_dsl) do
      erbh(<<-ERB)
        create_table "range_partitions", id: false, force: :cascade, options: "PARTITION BY RANGE(logdate)" do |t|
          t.integer "id", null: false
    ...
    let(:expected_dsl) do
      erbh(<<-ERB)
        ...
        add_partition "range_partitions", :range, [:logdate], partition_definitions: [{ name: "p0", values: {:from=>["MINVALUE"], :to=>["2021-01-01"]} }, {name: "pdefault", values: {:default=>true} }]
      ERB
    end

...
describe 'Ridgepole::Client#dump' do
  ...
  context 'when without table options' do
    let(:expected_dsl) do
      erbh(<<-ERB)
        create_table "books", <%= i cond('>= 6.1', id: { type: :bigint, unsigned: true }) %>, <%= i cond('>= 5.1, < 6.1', id: :bigint) %>, <%= i cond('< 6.1', unsigned: true) %>, force: :cascade, comment: "\\"london\\" bridge \\"is\\" falling \\"down\\"" do |t|
          t.string   "title", null: false
          t.integer  "author_id", null: false
          t.datetime "created_at"
          t.datetime "updated_at"
